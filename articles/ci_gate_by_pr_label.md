...
### なぜ draft 状態ではなくラベルで制御するのか

「draft の PR では CI を回さず、Ready にしたら回す」という、GitHub の draft 機能を使ったシンプルな方法もあります（[後述](#draft-pr-ではどうなる？)）。ただこの方法には、**「CI を回したい」と「レビューを依頼したい（= Ready にする）」が分離できない**という弱点があります。

- Ready にした時点で必ず CI が走る（= CI だけ止めておく、ができない）
- 逆に draft のままでは CI を回せない（= レビュー前に一度 CI を通しておく、ができない）

ラベルで制御すると、この 2 つを独立にコントロールできます。「Ready だけど CI はまだ」「draft だけど CI は回す」が自由に選べるので、**CI の発火タイミングをレビュー状態と切り離して運用したいチーム**に向いています。逆に「CI 起動 ≒ レビュー準備完了」で運用が回るなら、draft 方式のほうがシンプルです。
